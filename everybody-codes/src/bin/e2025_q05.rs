...
fn main() -> io::Result<()> {
    ...
    let (_, p1_input) = p1_input_str.split_once(':').unwrap();
    let p1_nums: Vec<_> = p1_input
        .split(',')
        .map(|n| n.parse::<usize>().unwrap())
        .collect();

    let p1_bone = build_fishbone(&p1_nums);
    ...
    let p2_bone_qualities: Vec<_> = p2_input
        ...
        .map(|l| {
            let nums: Vec<_> = l
                ...
                .map(|n| n.parse::<usize>().unwrap())
                .collect();
            get_quality(&build_fishbone(&nums))
        })
        .collect();
    ...
    let mut p3_bones: Vec<_> = p3_input
        .lines()
        .map(|l| {
            let (id, num_str) = l.split_once(':').unwrap();
            let nums: Vec<_> = num_str
                .split(',')
                .map(|n| n.parse::<usize>().unwrap())
                .collect();
            (id.parse::<usize>().unwrap(), build_fishbone(&nums))
        })
        .collect();
