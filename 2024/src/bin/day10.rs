...
fn count_trails(grid: &[Vec<u32>], start: Coord) -> (usize, usize) {
    let mut to_search = vec![start];
    let mut trail_ends: Vec<Coord> = vec![];
    while !to_search.is_empty() {
        let c = to_search.remove(0); // vecdeque?
...
fn get_all_trails(grid: &[Vec<u32>]) -> (usize, usize) {
...
fn main() -> io::Result<()> {
    let input: Vec<Vec<u32>> = get_grid(&fs::read_to_string(
