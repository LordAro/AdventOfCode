...
fn get_next_coord(c: &Coord, d: Dir, max_dim: &Coord) -> Option<Coord> {
    match d {
        Dir::North => {
            if c.y > 0 {
...
fn get_next_coord_on_loop(map: &Vec<Vec<char>>, pos: &Coord, dir: Dir) -> Option<(Coord, Dir)> {
    ...
    if let Some(next_coord) = get_next_coord(pos, dir, &max_dim) {
...
fn trace_route(map: &Vec<Vec<char>>, route: &[(Coord, Dir)]) -> (HashSet<Coord>, HashSet<Coord>) {
    let route_set: HashSet<Coord> = route.iter().map(|(c, _)| c).copied().collect(); // quick lookups
    ...
    for (c, move_dir) in route {
        ...
        if let Some(look_coord_a) = get_next_coord(c, look_dir_a, &max_dim) {
            if !route_set.contains(&look_coord_a) {
                set_a.insert(look_coord_a);
            }
        }
        if let Some(look_coord_b) = get_next_coord(c, look_dir_b, &max_dim) {
...
fn get_enclosed_pieces(map: &Vec<Vec<char>>, route: &[(Coord, Dir)]) -> HashSet<Coord> {
    ...
    assert!(filled_set_a.intersection(&filled_set_b).count() == 0);

    if filled_set_a
        .iter()
        .any(|c| c.x == 0 || c.y == 0 || c.x == map[0].len() - 1 || c.y == map.len() - 1)
    {
        // found an edge piece, must be the other
        filled_set_b
    } else {
        filled_set_a
    }
