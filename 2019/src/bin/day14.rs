...
type ReactionMap<'a> = HashMap<&'a str, (u32, Vec<(&'a str, u32)>)>;
    ...
    input_str
        ...
        .map(|s| {
            ...
            let inputs = in_out
                ...
                .map(|s| {
                    let mut ins = s.split(" ");
                    let in_n = ins.next().unwrap().parse::<u32>().unwrap();
            ...
            let output_n = out.next().unwrap().parse::<u32>().unwrap();
...
fn required_ore_count(reactions: &ReactionMap) -> u32 {
...
2 AB, 3 BC, 4 CA => 1 FUEL"
    ...
    #[test]
    fn ex3() {
        let input_str: Vec<_> = "157 ORE => 5 NZVS
165 ORE => 6 DCFZ
...
3 DCFZ, 7 NZVS, 5 HKGWZ, 10 PSHF => 8 KHKGT"
    ...
    #[test]
    fn ex4() {
        let input_str: Vec<_> = "2 VPVL, 7 FWMGM, 2 CXFTF, 11 MNCFX => 1 STKFG
...
176 ORE => 6 VJHF"
    ...
    #[test]
    fn ex5() {
        let input_str: Vec<_> = "171 ORE => 8 CNZTR
